...
#ui.title("Curriculum Design")
ui.page(title="Curriculum Design", path='/')
# with ui.row().classes('w-full justify-center'):
with ui.column().classes('w-full items-center'):
    with ui.row():
        ui.label('School of Technology Curriculum').classes('text-h3')
    with ui.row():
        ui.label('Complete tasks within each of the tabs shown below to generate curriculum report').classes('text-h6')
    ...
    with ui.step('Report Generation'):
        ...
        def get_plot():
            ...
            with container:
                ...
                with ui.column().classes('w-full items-center'):
                    with ui.row():
                        ui.label('Final Curriculum Report').classes('text-h4')
                plot_pie()
                ...
                with ui.grid(columns=2):
                    ...
                    def change_sel(e,i):
                        ...
                        if rows:
                            ...
                            li1[e-1].add_rows({"Category":rows[0]['Category'],"sub_name":rows[0]['sub_name'],"sub_code":rows[0]['sub_code'],"sub_credits":rows[0]['sub_credits']})
                            li1[i].remove_rows({"Category":rows[0]['Category'],"sub_name":rows[0]['sub_name'],"sub_code":rows[0]['sub_code'],"sub_credits":rows[0]['sub_credits']})
                            #lii2[i].remove_rows({"Category":i,"sub_name":rows[0]['sub_name'],"sub_code":rows[0]['sub_code'],"sub_credits":rows[0]['sub_credits']})
                        #ui.notify(rows)
                        else:
                            ui.notify("No rows selected")
                    x = 0
                    ...
                    for i in df_list:
                        table = "table"+str(20+x)
                        select = "select"+str(20+x)
                        # df =  pd.DataFrame(data={'col1': [1, 2], 'col2': [3, 4]})
                        with ui.table(title="Semester - "+str(x+1),columns=[{'name': col, 'label': col, 'field': col} for col in i.columns],
                        rows=i.to_dict('records'),row_key='sub_code', selection='single') as table:
                            with table.add_slot('bottom-row'):
                                with table.row():
                                    with table.cell():
                                        ui.label()
                                    with table.cell():
                                        sem = [1,2,3,4,5,6,7,8]
                                        sem.remove(x+1)
                                        select = ui.select(sem,label='Move to' , value="Move to",on_change= lambda e,x=x:change_sel(e.value,x))
                ...
                #         rows = lii1[i].selected
                #         if rows:
                #             lii1[i].remove_rows({"Category":i,"sub_name":rows[0]['sub_name'],"sub_code":rows[0]['sub_code'],"sub_credits":rows[0]['sub_credits']})
                #             #lii2[i].remove_rows({"Category":i,"sub_name":rows[0]['sub_name'],"sub_code":rows[0]['sub_code'],"sub_credits":rows[0]['sub_credits']})
                #         #ui.notify(rows)
                #     x = 0
                ...
                #                         select = ui.select(sem,label='Move to' , value="Move to",on_change= lambda i=i:change_sel(str(i)))
                #         lii1[str(i)]=table
                #         lii2[str(i)]=select
                #         x+=1  
                ...
                #df1 = pd.read_csv("Final_Report.csv")
                    ...
                    pdf = matplotlib.backends.backend_pdf.PdfPages("Semester_"+str(mm)+".pdf")
                    pdf.savefig(fig)
                    mm+=1
                    pdf.close()
        # def save_pdf():
        ...
        #     pdfkit.configuration(wkhtmltopdf=r'wkhtmltopdf.exe')
        #     pdfkit.from_string(html_table, 'Final.pdf', options=options)
        #Child/SUb Stepper Begin
        btn_plot = ui.button("Generate Final Report and plot", on_click = get_plot, color='orange')
        # btn_pdf = ui.button("Save as PDF", on_click = save_pdf, color='orange')
        
        # async def alert():
        #     await ui.run_javascript('window.print()', respond=False)
        # btn_print = ui.button("Print", on_click = alert, color='orange')
...
container =ui.row()
#ui.run(reload=False, native=True, window_size=(1000, 800), fullscreen=False) 
with ui.footer():
    with ui.column().classes('w-full items-center'):
        with ui.row():
            ui.markdown("Developed by **Lokesh Kumar** and **Amogh Deshmukh**").classes('text-h6')
ui.run(port=8085,title="Curriculum Design", favicon='🚀')
